...
# Función para obtener la herramienta
def getTool(x):
    if x < 50 + ml:
        return "linea_Recta"
    elif x < 100 + ml:
...
# Función para verificar si el índice está levantado
...
# Máscara de un solo canal y tipo uint8
...
while True:
    ...
    if mask.shape[:2] != frm.shape[:2]:
        mask = cv2.resize(mask, (frm.shape[1], frm.shape[0]))

    # Aplicar la operación bitwise_and
    op = cv2.bitwise_and(frm, frm, mask=mask)

    # Actualizar los canales de color
    frm[:, :, 1] = op[:, :, 1]
    frm[:, :, 2] = op[:, :, 2]
